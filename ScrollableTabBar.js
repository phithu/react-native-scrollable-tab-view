...
const ScrollableTabBar = createReactClass({
  ...
  updateTabPanel(position, pageOffset) {
    ...
    newScrollX -= (containerWidth - (1 - pageOffset) * tabWidth - pageOffset *
      nextTabWidth) / 2;
  ...
  updateTabUnderline(position, pageOffset, tabCount) {
    ...
    if (position < tabCount - 1) {
      ...
      const newLineLeft = (pageOffset * nextTabLeft + (1 - pageOffset) *
        lineLeft);
      const newLineRight = (pageOffset * nextTabRight + (1 - pageOffset) *
        lineRight);
  ...
  measureTab(page, event) {
    ...
    const value = this.props.scrollValue.__getValue();
